...
export default async function handler(req, res) {
  ...
  if (!sub) return res.status(401).json({ error: "Please sign in first" });
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (session.metadata?.sub !== sub) return res.status(403).json({ error: "This order belongs to a different account" });
    ...
    p.set("line_items[0][price_data][product_data][name]", `FluffBooth · ${pack.name}`);
