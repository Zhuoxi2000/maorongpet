...
浏览器直接打开（`ADMIN_KEY` 就是看板用的那个环境变量的值）：

```
https://fluffbooth.com/api/grant?key=ADMIN_KEY的值&email=对方邮箱&credits=20
```

- 对方需要**先用 Google 登录过一次**（额度挂在账号上），否则返回 404
- 发的是"奖励额度"：永不过期，每日免费用完后自动消耗
- `credits` 可为负数（发错了扣回），单次限 ±1000
- 返回 JSON 里的 `balance` 是发放后的最新余额
